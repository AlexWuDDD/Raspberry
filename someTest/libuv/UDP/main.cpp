//
// Created by wkjwo on 9/19/2019.
//
//Setup and send UDP packets
#include <cstdlib>
#include <cstdio>
#include <cstring>
#include "uv.h"
...
int main()
{
    ...
    uv_udp_bind(&recv_socket,
            (const struct sockaddr*)&recv_addr, UV_UDP_REUSEADDR);
    uv_udp_recv_start(&recv_socket, alloc_buffer, on_read);

    uv_udp_init(loop, &send_socket);
    struct sockaddr_in broadcast_addr;
    uv_ip4_addr("0,0,0,0", 0, &broadcast_addr);
    uv_udp_bind(&send_socket,
                (const struct sockaddr*)&broadcast_addr, 0);
